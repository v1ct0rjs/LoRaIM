...
MQTT_USERNAME = os.getenv("MQTT_USERNAME", "akyvqaco")
MQTT_PASSWORD = os.getenv("MQTT_PASSWORD", "iJ9JikQMjJkc")
        ...
        if node_id_from_message and node_id_from_message not in ["web_client", "sent", "?", "desconocido",
            ...
            asyncio.run_coroutine_threadsafe(
                manager.schedule_broadcast(
                    json.dumps({"type": "node_update", "data": NODES_STATUS[node_id_from_message]})),
...
def mqtt_connect_client():
    ...
    # Corrected TLS logic:
    if MQTT_BROKER == "test.mosquitto.org":
        if MQTT_PORT == 8883:  # TLS port for test.mosquitto.org
            logger.info(f"Connecting to {MQTT_BROKER}:{MQTT_PORT}, enabling TLS.")
            client.tls_set(tls_version=mqtt.client.ssl.PROTOCOL_TLS)
        elif MQTT_PORT == 1883:  # Non-TLS port for test.mosquitto.org
            logger.info(f"Connecting to {MQTT_BROKER}:{MQTT_PORT}, TLS not enabled.")
            # No client.tls_set() call
        else:
            logger.warning(
                f"Connecting to {MQTT_BROKER} on an unrecognized port {MQTT_PORT}. Assuming no TLS. Connection might fail or be insecure.")
    elif MQTT_PORT == 8883:  # Standard TLS port for other brokers
        logger.info(f"MQTT Port is {MQTT_PORT} (standard TLS), enabling TLS.")
        client.tls_set(tls_version=mqtt.client.ssl.PROTOCOL_TLS)
    else:  # Default for other brokers on non-8883 ports: no TLS
        logger.info(f"Connecting to {MQTT_BROKER}:{MQTT_PORT}. TLS not enabled by default for this port.")
